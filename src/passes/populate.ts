...
export function populate(board: Board, self: Battlesnake): number[][] {
    ...
    let gameState = new Array(board.width).fill(new Array(board.height).fill(1));
    ...
    for (let snake of board.snakes) {
        ...
        gameState[snake.body[-1].x][snake.body[-1].y] = 1;
        ...
        for (let x = 0; x < 1; x++) {
            for (let y = 0; y < 1; y++) {
                ...
                if (gameState[x] == undefined) continue;
                if (gameState[x][y] == undefined) continue;
                gameState[x][y] = 0;
            }
        }
